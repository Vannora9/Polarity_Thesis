...
class MyTaskProcessor():
  ...
  def _create_examples(self, lines, set_type):
    ...
    examples = []
    for (id, i, line) in enumerate(lines):
      guid = '%s-%s' %(set_type, i)
...
class DataTrainingArguments:
    ...
    def __post_init__(self):
        ...
        ### Revised:
            ...
            raise ValueError("Need either a GLUE task or a training file.")
        else:
...
def main():
    ...
    if data_args.task_name is not None:
        ...
        is_regression = datasets["train"].features["label"].dtype in ["float32", "str"]
        if is_regression:
            num_labels = 1
        else:
    ...
    if training_args.do_predict:
        ...
        for test_dataset, task in zip(test_datasets, tasks):
            ...
            from_flax=True
            ### Revised
            if show_prob_flag: predictions_org = predictions
                ...
                with open(output_test_file, "w") as writer:
                    ...
                    for index, item in enumerate(predictions):
                        if is_regression:
                            ...
                            ### Revised
                            if show_prob_flag:
                                probs = " ".join(map(str,predictions_org[index]))
                                #max_prob = str(np.max(predictions_org[index]))
                                writer.write("%d\t%s\t%s\n" % (index, item, probs))
                            else:
